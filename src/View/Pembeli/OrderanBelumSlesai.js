...
class OrderBelumTerkirim extends React.Component{
    constructor(){
     ...
     componentDidMount(){
        ...
        fetch(`http://localhost:4000/pesanan/pembeli/${decoded.id_user}/status/Belum dikirim`,{
            method:"GET",
            header:header,
        })
        .then(res=> res.json())
     ...
     yesnoCheck(){
      if (document.getElementById("diterima").checked){
        this.state.form.Status="Cancel"
      }else{
        this.state.form.Status="Belum dikirim"
      }
    }
    UpdateStatus=()=>{
      const PesananData ={
      ...
      <Table className={classes.table} aria-label="customized table">
        <TableHead>
          <TableRow>
            ...
            <StyledTableCell align="right">Nama Toko</StyledTableCell>
        ...
        <TableBody>
          {row.map(rows => (
            <StyledTableRow key={row.name}>
              ...
              <StyledTableCell align="right"><IconButton aria-label="delete" onClick={()=> {this.selectPesanan(rows); this.ModalEdit()}}><CancelIcon/></IconButton></StyledTableCell>
            </StyledTableRow>
          ))}
        </TableBody>
      </Table>
    ...
    <Modal isOpen={this.state.ModalEdit}>
                    ...
                    <ModalBody>
                        <div className="form-group">
                            ...
                            <br/>
                            <label>Cancel barang ini? </label> &nbsp;&nbsp;
                            <Form>
                            <Form.Group controlId="diterima">
                                  <Form.Check value="diterima" id ="diterima" controlId="diterima" type="checkbox" label="Ya" onClick={this.yesnoCheck} onChange={this.handleChange} value={form?form.Status:''} />
                              </Form.Group>
                            </Form>
